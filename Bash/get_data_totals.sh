...
# exception to starting with a '/' is if you are using a shortcut like '~', which will
# expand to have a '/' at the start. The use of blank lines as whitespace is optional.
#
...
declare -a VOL_NAMES=() # parallel array with next three
...
while [ "x${DIR_PATHS[$a]}" != "x" ]; do
   ...
   let SIZE_SECTORS=`sudo du -sc "$FULL_PATH" 2> /dev/null | tail -1 | cut -f 1`
...
while [ "x${DIR_PATHS[$a]}" != "x" ]; do
   ...
   if [ ${VOL_NAMES[$a]} != $LAST_VOL ]; then
      ...
      echo -ne "Volume: ${VOL_NAMES[$a]} ($USED/$CAPAC GB used)"
